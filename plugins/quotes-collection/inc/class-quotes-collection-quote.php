...
class Quotes_Collection_Quote {
    ...
    $this->quote_id = $this->text_format ($this->quote_id);
	}
	...
	public function output_format( $options = array() ) {
      ...
      'show_quote_id' => 1,
		...
		if( $options['show_author'] && $this->author ) {
			$attribution = '<cite class="author quote_id-' . $this->quote_id . '"><button id="author-button" class="popmake-quote1">' . $this->author . '</button></cite>';
		}
